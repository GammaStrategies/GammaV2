...
contract SimpleLensToken1OnlyTest is Test, Deployers {
    ...
    function setUp() public {
        ...
        // Setup initial liquidity with token1-only positions
        _setupToken1OnlyPositions();
    }

    function _setupToken1OnlyPositions() internal {
        // Mint tokens to alice (we need some token0 for initial deposit even if positions will be token1-only)
        token0.mint(alice, 1000e18);
        token1.mint(alice, 1000e18);

        // Alice deposits - need to deposit both tokens initially
        vm.startPrank(alice);
        token0.approve(address(mpm), type(uint256).max);
        token1.approve(address(mpm), type(uint256).max);

        // Deposit a reasonable amount to have liquidity to work with
        mpm.deposit(140e18, 140e18, alice, alice);

        // Create three positions all below current price (token1 only)
        // Position 0: -180 to -120 (target: 80 token1)
        // Position 1: -120 to -60 (target: 40 token1)
        // Position 2: -60 to 0 (target: 20 token1)
        ...
        ranges[2].upperTick = 0;

        // Calculate liquidities to get roughly 80, 40, and 20 token1 in each position
        // Since these are all below current price, they'll only contain token1
        uint128[] memory liquidities = new uint128[](3);

        // For position below current price, only token1 is used
        liquidities[0] = LiquidityAmounts.getLiquidityForAmounts(
            INITIAL_PRICE_SQRT, TickMath.getSqrtPriceAtTick(-180), TickMath.getSqrtPriceAtTick(-120), 0, 80e18
        );
        liquidities[1] = LiquidityAmounts.getLiquidityForAmounts(
            INITIAL_PRICE_SQRT, TickMath.getSqrtPriceAtTick(-120), TickMath.getSqrtPriceAtTick(-60), 0, 40e18
        );
        liquidities[2] = LiquidityAmounts.getLiquidityForAmounts(
            INITIAL_PRICE_SQRT, TickMath.getSqrtPriceAtTick(-60), TickMath.getSqrtPriceAtTick(0), 0, 20e18
        );

        // Rebalance to create these positions
        IMultiPositionManager.RebalanceParams memory paramsToken1 = IMultiPositionManager.RebalanceParams({
        ...
        (uint256[2][] memory outMinToken1, uint256[2][] memory inMinToken1) =         SimpleLensInMin.getOutMinAndInMinForRebalance(
            mpm,
            paramsToken1.strategy,
            paramsToken1.center,
            paramsToken1.tLeft,
            paramsToken1.tRight,
            paramsToken1.limitWidth,
            paramsToken1.weight0,
            paramsToken1.weight1,
            paramsToken1.useCarpet,
            false,
            500,
            500
        );

        mpm.rebalance(paramsToken1, outMinToken1, inMinToken1);
        vm.stopPrank();
    }

    function test_Token1OnlyWithdrawal() public {
        console.log("=== TESTING TOKEN1-ONLY POSITIONS WITHDRAWAL ===\n");
        ...
        (IMultiPositionManager.Range[] memory initialPositions, IMultiPositionManager.PositionData[] memory initialData)
        = mpm.getPositions();

        console.log("\nInitial positions (all below current price, token1-only):");
        for (uint256 i = 0; i < initialPositions.length; i++) {
            if (initialPositions[i].lowerTick != 0 || initialPositions[i].upperTick != 0) {
                console.log("  Position", i);
                console.logInt(initialPositions[i].lowerTick);
                console.logInt(initialPositions[i].upperTick);
                console.log("    Token0:", initialData[i].amount0);
                console.log("    Token1:", initialData[i].amount1);
            }
        }

        // Preview withdrawal of 70 token1
        uint256 withdrawAmount = 70e18;
        console.log("\n=== WITHDRAWING 70 TOKEN1 ===");
        ...
        console.log("  Token0 change:", preview.token0Change);
        console.log("  Token1 change:", preview.token1Change);
        console.log("  New total token0:", preview.newTotalToken0);
        console.log("  New total token1:", preview.newTotalToken1);
        */

        /* Commented out - needs update for new signature
        // Log preview position details
        console.log("\nPreview - Positions after withdrawal:");
        for (uint i = 0; i < preview.newPositions.length; i++) {
            if (preview.newPositions[i].tickLower != 0 || preview.newPositions[i].tickUpper != 0) {
                console.log("  Position", i);
                console.logInt(preview.newPositions[i].tickLower);
                console.logInt(preview.newPositions[i].tickUpper);
                console.log("    Token0:", preview.newPositions[i].token0Quantity);
                console.log("    Token1:", preview.newPositions[i].token1Quantity);
            }
        }
        */
        ...
        {
            vm.startPrank(alice);
            // Get proper outMin from previewWithdrawCustom
        ...
        console.log("\nActual Withdrawal:");
        console.log("  Amount withdrawn:", amount1Out);
        ...
        for (uint256 i = 0; i < actualPositions.length; i++) {
            if (actualPositions[i].lowerTick != 0 || actualPositions[i].upperTick != 0) {
                ...
                console.log("    Token0:", actualData[i].amount0);
                console.log("    Token1:", actualData[i].amount1);
            }
        }
        ...
        */
        ...
        for (uint i = 0; i < actualPositions.length && i < preview.newPositions.length; i++) {
            ...
            uint256 token0Diff = preview.newPositions[i].token0Quantity > actualData[i].amount0 ?
                preview.newPositions[i].token0Quantity - actualData[i].amount0 :
                actualData[i].amount0 - preview.newPositions[i].token0Quantity;
            uint256 token1Diff = preview.newPositions[i].token1Quantity > actualData[i].amount1 ?
                preview.newPositions[i].token1Quantity - actualData[i].amount1 :
                actualData[i].amount1 - preview.newPositions[i].token1Quantity;
        ...
        */
        ...
        if (positionsMatch) {
            console.log("\n[PASS] All positions match within tolerance");
        }
        ...
        assertEq(amount1Out, withdrawAmount, "Should withdraw exact amount");
